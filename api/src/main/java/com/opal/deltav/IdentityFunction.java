...
/**
 * Verifies patient identity (name + DOB) and sends OTP to the phone on file.
 * OTP is never sent on a failed identity match.
 */
public class IdentityFunction {

    private static final Gson gson = new Gson();

    @FunctionName("identityVerify")
    public HttpResponseMessage verifyIdentity(
            @HttpTrigger(
                    name = "req",
                    methods = {HttpMethod.POST, HttpMethod.OPTIONS},
                    authLevel = AuthorizationLevel.ANONYMOUS,
                    route = "api/identity/verify"
        ...
        String firstName = getString(json, "first_name");
        String lastName = getString(json, "last_name");
        String dobRaw = getString(json, "dob");

        LocalDate dob;
        try {
            dob = LocalDate.parse(dobRaw);
        } catch (Exception e) {
            return jsonResponse(request, HttpStatus.BAD_REQUEST, Map.of("error", "invalid_dob"));
        }

        try {
            PatientIdentityService.LookupResult lookup =
                    PatientIdentityService.lookup(practice, firstName, lastName, dob);
            if (!lookup.matched) {
                if ("no_phone_on_file".equals(lookup.error)) {
                    return jsonResponse(request, HttpStatus.UNPROCESSABLE_ENTITY,
                            Map.of("error", lookup.error,
                                    "message", "We found your record but no mobile number is on file. Please contact the office."));
                }
                return jsonResponse(request, HttpStatus.NOT_FOUND,
                        Map.of("error", "identity_mismatch",
                                "message", "We could not verify your information. Please check your details and try again."));
            }

            String sessionId = RedisSessionService.createSession(practice);
            String phoneE164 = PhoneUtil.toE164(lookup.patient.phone);
            RegistrationSession session = RedisSessionService.bindPatient(
                    sessionId, RedisOtpService.normalizePracticeId(practice),
                    lookup.patient.patientKey, phoneE164);

            return deliverOtp(request, logger, session, false);
        } catch (IllegalStateException e) {
            logger.severe("Identity verify configuration error: " + e.getMessage());
            return jsonResponse(request, HttpStatus.INTERNAL_SERVER_ERROR, Map.of("error", "Server configuration error"));
        } catch (Exception e) {
            logger.severe("Identity verify failed: " + e.getClass().getName() + " - " + e.getMessage());
            return jsonResponse(request, HttpStatus.INTERNAL_SERVER_ERROR, Map.of("error", "Failed to verify identity"));
        }
    }

    static HttpResponseMessage deliverOtp(HttpRequestMessage<Optional<String>> request, Logger logger,
